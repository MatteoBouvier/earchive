...
def invalid_paths(
    ...
    for root, dirs, files in progress(paths):
        for file in filter(lambda f: not _is_excluded(f, config), map(lambda f: root / f, files)):
            yield from _check_valid_file(file, config, checks)

        for dir in filter(lambda d: not _is_excluded(d, config), map(lambda d: root / d, dirs)):
...
def fix_invalid_paths(
    config: Config, progress: Bar[tuple[FastPath, list[str], list[str]]], counter: Counter
) -> Generator[PathDiagnostic, None, None]:
    _empty_dirs: set[FastPath] = set()
    errors: list[PathDiagnostic] = []
    ...
    for root, dirs, files in progress(paths):
        for file_path in filter(lambda f: not _is_excluded(f, config), map(lambda f: root / f, files)):
